...
float compute_dt_gpu_half(const half* dU, int NX, int NY, float dx, float dy, float dt_scale){
    ...
    reset_maxspeed<<<1,1>>>();
    kernel_maxspeed_half<<<grid,block>>>(dU,NX,NY);
...
static void dump_field(const std::vector<float>& data, const char* tag, int step){
...
int main(int argc, char** argv){
    ...
    // Convert initial U (float) to half on device.
    ...
    for (int s=0; s<STEPS; ++s){
        float dt = compute_dt_gpu_half(dU, NX, NY, dx, dy, dt_scale);

        step_mhd_rusanov_half<<<grid,block>>>(dU,dV,NX,NY,dx,dy,dt);
        CHECK_LAST();
        CHECK_CUDA(cudaDeviceSynchronize());
        std::swap(dU,dV);

        // Extract rho to float and solve Poisson.
        ...
        // Apply Poisson gradient to momentum.
        apply_grad_phi_half<<<grid,block>>>(dU, d_phi, NX, NY, dx, dy, dt);
        CHECK_LAST();
        CHECK_CUDA(cudaDeviceSynchronize());
        ...
        if (dump_every > 0 && (s % dump_every) == 0){
            ...
            dump_field(h_rho, "rho", s);
            dump_field(h_phi, "phi", s);
        }

        if ((s % 10) == 0 || s == STEPS - 1){
            float rmin = *std::min_element(h_rho.begin(), h_rho.end());
            float rmax = *std::max_element(h_rho.begin(), h_rho.end());
            std::printf("Step %4d/%4d  dt=%.3e  rho[min,max]=[%.4f, %.4f]\n",
                        s, STEPS, dt, rmin, rmax);
        }
    }
