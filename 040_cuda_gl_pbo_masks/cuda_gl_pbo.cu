...
__global__ void fill_pbo(uchar4* pbo, int w, int h){
    int x = blockIdx.x * blockDim.x + threadIdx.x;
    int y = blockIdx.y * blockDim.y + threadIdx.y;
    if (x >= w || y >= h) return;
    int idx = y * w + x;
    pbo[idx] = make_uchar4((x ^ y) & 255, (x * 3) & 255, (y * 5) & 255, 255);
}

int main(){
    if (!glfwInit()) return 1;
    GLFWwindow* window = glfwCreateWindow(640, 480, "PBO", nullptr, nullptr);
    ...
    fill_pbo<<<grid, block>>>(dev_ptr, 640, 480);
