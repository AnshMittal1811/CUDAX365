...
constexpr float GAMMA=1.4f, RHO_MIN=1e-6f, P_MIN=1e-6f, CFL=0.25f;
...
__global__ void refine_patch(const float* Uin, float* Uout,
                             int NX,int NY,float dx,float dy,float dt,
                             int x0,int y0,int w,int h){
    int ix = x0 + threadIdx.x;
    int iy = y0 + threadIdx.y;
    if (threadIdx.x>=w || threadIdx.y>=h || ix>=NX || iy>=NY) return;
    int idx = iy*NX + ix;
...
__global__ void step_mhd_rusanov_dynpar(const float* Uin, float* Uout,
                                        int NX,int NY,float dx,float dy,float dt,
                                        float refine_thresh)
{
    int ix = blockIdx.x*blockDim.x + threadIdx.x;
    int iy = blockIdx.y*blockDim.y + threadIdx.y;
    bool in_bounds = (ix < NX && iy < NY);
    if (in_bounds){
        int idx = iy*NX + ix;
        int im1=wrap(ix-1,NX), ip1=wrap(ix+1,NX);
        int jm1=wrap(iy-1,NY), jp1=wrap(iy+1,NY);
    ...
    __syncthreads();
    if (refine_thresh > 0.f && threadIdx.x==0 && threadIdx.y==0){
        int cx = blockIdx.x*blockDim.x + blockDim.x/2;
        int cy = blockIdx.y*blockDim.y + blockDim.y/2;
        if (cx < NX && cy < NY){
            float rc = Uin[0*NX*NY + cy*NX + cx];
            float r1 = Uin[0*NX*NY + cy*NX + wrap(cx+1,NX)];
            float r2 = Uin[0*NX*NY + wrap(cy+1,NY)*NX + cx];
            float grad = fabsf(r1-rc) + fabsf(r2-rc);
            if (grad > refine_thresh){
                int x0 = blockIdx.x*blockDim.x;
                int y0 = blockIdx.y*blockDim.y;
                refine_patch<<<dim3(1,1), blockDim>>>(Uin,Uout,NX,NY,dx,dy,0.5f*dt,x0,y0,blockDim.x,blockDim.y);
            }
        }
    }
}
...
float compute_dt_gpu(const float* dU, int NX, int NY, float dx, float dy){
    dim3 block(BLOCK_X,BLOCK_Y), grid((NX+BLOCK_X-1)/BLOCK_X,(NY+BLOCK_Y-1)/BLOCK_Y);
    reset_maxspeed<<<1,1>>>();
...
static void dump_field(const char* dir, const char* tag, int step, const std::vector<float>& data){
    std::filesystem::create_directories(dir);
    char name[256];
    std::snprintf(name, sizeof(name), "%s/%s_%04d.bin", dir, tag, step);
    FILE* fp = std::fopen(name, "wb");
    if (fp){ std::fwrite(data.data(), sizeof(float), data.size(), fp); std::fclose(fp); }
}

static void run_sim(const char* out_dir, float refine_thresh,
                    int NX,int NY,int STEPS,int dump_every)
{
    float Lx=1.0f, Ly=1.0f, dx=Lx/NX, dy=Ly/NY;
    size_t cells=(size_t)NX*NY, bytes=6*cells*sizeof(float);
    std::vector<float> hU(6*cells), h_rho(cells), h_phi(cells), h_bx(cells), h_by(cells);
    ...
    for (int s=0; s<STEPS; ++s){
        ...
        step_mhd_rusanov_dynpar<<<grid,block>>>(dU,dV,NX,NY,dx,dy,dt,refine_thresh);
        CHECK_CUDA(cudaDeviceSynchronize());
        std::swap(dU,dV);
        // mean rho on host
        ...
        if (dump_every > 0 && (s % dump_every) == 0){
            CHECK_CUDA(cudaMemcpy(h_rho.data(), dU + 0*cells, cells*sizeof(float), cudaMemcpyDeviceToHost));
            CHECK_CUDA(cudaMemcpy(h_phi.data(), d_phi, cells*sizeof(float), cudaMemcpyDeviceToHost));
            CHECK_CUDA(cudaMemcpy(h_bx.data(), dU + 3*cells, cells*sizeof(float), cudaMemcpyDeviceToHost));
            CHECK_CUDA(cudaMemcpy(h_by.data(), dU + 4*cells, cells*sizeof(float), cudaMemcpyDeviceToHost));
            dump_field(out_dir, "rho", s, h_rho);
            dump_field(out_dir, "phi", s, h_phi);
            dump_field(out_dir, "bx",  s, h_bx);
            dump_field(out_dir, "by",  s, h_by);
        }
    }
    poisson.destroy();
...
int main(int argc,char** argv){
    ...
    float refine_thresh=(argc>4)?atof(argv[4]):0.15f;
    int dump_every=(argc>5)?atoi(argv[5]):1;
    run_sim("frames_base",  -1.0f, NX, NY, STEPS, dump_every);
    run_sim("frames_refine", refine_thresh, NX, NY, STEPS, dump_every);
    return 0;
}
