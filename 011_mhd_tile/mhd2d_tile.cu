...
#define TILE_Y 16
#endif
#ifndef PAD_X   // set to 1 to avoid bank conflicts in shared tiles
#define PAD_X 1
#endif
...
// Shared tile extents include a 1-cell halo on each side.
template<int TX,int TY,int PAD>
__global__ void step_mhd_tiled(
    const float* __restrict__ Uin,
    float* __restrict__ Uout,
    int NX,int NY,float dx,float dy,float dt)
{
    constexpr int SX = TX + 2 + PAD;   // +2 halos, +PAD for bank-conflict avoidance
    constexpr int SY = TY + 2;

    // six planes in shared memory (SoA), padded x-dimension
    __shared__ float sh_rho[SY][SX];
    __shared__ float sh_mx [SY][SX];
    __shared__ float sh_my [SY][SX];
    __shared__ float sh_bx [SY][SX];
    __shared__ float sh_by [SY][SX];
    __shared__ float sh_E  [SY][SX];

    int ix = blockIdx.x*TX + threadIdx.x;
    int iy = blockIdx.y*TY + threadIdx.y;

    // local (tile) indices shifted by +1 to leave room for halo
    ...
    // 1) load interior cell
    if (ix<NX && iy<NY){
        sh_rho[ty][tx] = gAt(0,ix,iy);
        sh_mx [ty][tx] = gAt(1,ix,iy);
        sh_my [ty][tx] = gAt(2,ix,iy);
        sh_bx [ty][tx] = gAt(3,ix,iy);
        sh_by [ty][tx] = gAt(4,ix,iy);
        sh_E  [ty][tx] = gAt(5,ix,iy);
    }

    // 2) load halos along x
    if (threadIdx.x==0){
        int gx=wrap(ix-1,NX), gy=iy;
        if (gy<NY){
            sh_rho[ty][0] = gAt(0,gx,gy);
            sh_mx [ty][0] = gAt(1,gx,gy);
            sh_my [ty][0] = gAt(2,gx,gy);
            sh_bx [ty][0] = gAt(3,gx,gy);
            sh_by [ty][0] = gAt(4,gx,gy);
            sh_E  [ty][0] = gAt(5,gx,gy);
        }
    }
    if (threadIdx.x==TX-1){
        int gx=wrap(ix+1,NX), gy=iy;
        if (gy<NY){
            sh_rho[ty][TX+1] = gAt(0,gx,gy);
            sh_mx [ty][TX+1] = gAt(1,gx,gy);
            sh_my [ty][TX+1] = gAt(2,gx,gy);
            sh_bx [ty][TX+1] = gAt(3,gx,gy);
            sh_by [ty][TX+1] = gAt(4,gx,gy);
            sh_E  [ty][TX+1] = gAt(5,gx,gy);
        }
    }

    // 3) load halos along y
    if (threadIdx.y==0){
        int gy=wrap(iy-1,NY), gx=ix;
        if (gx<NX){
            sh_rho[0][tx] = gAt(0,gx,gy);
            sh_mx [0][tx] = gAt(1,gx,gy);
            sh_my [0][tx] = gAt(2,gx,gy);
            sh_bx [0][tx] = gAt(3,gx,gy);
            sh_by [0][tx] = gAt(4,gx,gy);
            sh_E  [0][tx] = gAt(5,gx,gy);
        }
    }
    if (threadIdx.y==TY-1){
        int gy=wrap(iy+1,NY), gx=ix;
        if (gx<NX){
            sh_rho[TY+1][tx] = gAt(0,gx,gy);
            sh_mx [TY+1][tx] = gAt(1,gx,gy);
            sh_my [TY+1][tx] = gAt(2,gx,gy);
            sh_bx [TY+1][tx] = gAt(3,gx,gy);
            sh_by [TY+1][tx] = gAt(4,gx,gy);
            sh_E  [TY+1][tx] = gAt(5,gx,gy);
        }
    }

    // 4) load corners (optional, for completeness)
    if (threadIdx.x==0 && threadIdx.y==0){
        int gx=wrap(ix-1,NX), gy=wrap(iy-1,NY);
        sh_rho[0][0]=gAt(0,gx,gy); sh_mx[0][0]=gAt(1,gx,gy); sh_my[0][0]=gAt(2,gx,gy);
        sh_bx[0][0]=gAt(3,gx,gy); sh_by[0][0]=gAt(4,gx,gy); sh_E [0][0]=gAt(5,gx,gy);
    }
    if (threadIdx.x==TX-1 && threadIdx.y==0){
        int gx=wrap(ix+1,NX), gy=wrap(iy-1,NY);
        sh_rho[0][TX+1]=gAt(0,gx,gy); sh_mx[0][TX+1]=gAt(1,gx,gy); sh_my[0][TX+1]=gAt(2,gx,gy);
        sh_bx[0][TX+1]=gAt(3,gx,gy); sh_by[0][TX+1]=gAt(4,gx,gy); sh_E [0][TX+1]=gAt(5,gx,gy);
    }
    if (threadIdx.x==0 && threadIdx.y==TY-1){
        int gx=wrap(ix-1,NX), gy=wrap(iy+1,NY);
        sh_rho[TY+1][0]=gAt(0,gx,gy); sh_mx[TY+1][0]=gAt(1,gx,gy); sh_my[TY+1][0]=gAt(2,gx,gy);
        sh_bx[TY+1][0]=gAt(3,gx,gy); sh_by[TY+1][0]=gAt(4,gx,gy); sh_E [TY+1][0]=gAt(5,gx,gy);
    }
    if (threadIdx.x==TX-1 && threadIdx.y==TY-1){
        int gx=wrap(ix+1,NX), gy=wrap(iy+1,NY);
        sh_rho[TY+1][TX+1]=gAt(0,gx,gy); sh_mx[TY+1][TX+1]=gAt(1,gx,gy); sh_my[TY+1][TX+1]=gAt(2,gx,gy);
        sh_bx[TY+1][TX+1]=gAt(3,gx,gy); sh_by[TY+1][TX+1]=gAt(4,gx,gy); sh_E [TY+1][TX+1]=gAt(5,gx,gy);
    }
    ...
    // Build left/right/up/down conservative states from shared mem
    ...
    rusanov(ULx, Uc , 0, FxL);
    rusanov(Uc , URx, 0, FxR);
    rusanov(UDy, Uc , 1, GyD);
    rusanov(Uc , UUy, 1, GyU);
...
// --- tiny CFL calculator (reuse from Day 10, simplified) ---
...
__global__ void kernel_maxspeed(const float* U,int NX,int NY){
    ...
    float r=safe_rho(rho), vx=mx/r, vy=my/r, p=pressure(rho,mx,my,bx,by,E), cf=cf_fast(rho,p,bx,by);
...
void init_orszag_tang(std::vector<float>& hU,int NX,int NY,float Lx,float Ly){
    ...
    for(int y=0;y<NY;y++) for(int x=0;x<NX;x++){
        ...
        at(0,x,y)=rho; at(1,x,y)=mx; at(2,x,y)=my; at(3,x,y)=bx; at(4,x,y)=by; at(5,x,y)=E;
    }
}
...
int main(int argc,char**argv){
    ...
    if (STEPS%2) std::swap(dU,dV);
    CHECK_CUDA(cudaMemcpy(hU.data(),dV,bytes,cudaMemcpyDeviceToHost));
