...
__device__ __forceinline__ unsigned long long atom_add_u64(unsigned long long* addr, unsigned long long val){
    unsigned long long old;
    asm volatile("atom.global.add.u64 %0, [%1], %2;" : "=l"(old) : "l"(addr), "l"(val) : "memory");
    return old;
}

__global__ void reward_counter_kernel(const float* rho, const float* phi,
                                      const float* bx, const float* by,
                                      int N, float rho_min, float rho_max,
                                      float phi_max, float b_max,
                                      unsigned long long* counter)
{
    ...
    float r = rho[i];
    float p = phi[i];
    float bmag = sqrtf(bx[i]*bx[i] + by[i]*by[i]);
    if (r >= rho_min && r <= rho_max && fabsf(p) <= phi_max && bmag <= b_max){
        atom_add_u64(counter, 1ull);
    }
}
...
int main(int argc,char** argv){
    int NX=128, NY=128, STEPS=100, reward_every=10;
    float cfl=0.25f;
    float rho_min=0.8f, rho_max=1.2f, phi_max=0.2f, b_max=1.5f;
    for (int i=1;i<argc;i++){
        if (std::strcmp(argv[i], "--cfl") == 0 && i+1 < argc){ cfl = std::atof(argv[++i]); }
        else if (std::strcmp(argv[i], "--rho-min") == 0 && i+1 < argc){ rho_min = std::atof(argv[++i]); }
        else if (std::strcmp(argv[i], "--rho-max") == 0 && i+1 < argc){ rho_max = std::atof(argv[++i]); }
        else if (std::strcmp(argv[i], "--phi-max") == 0 && i+1 < argc){ phi_max = std::atof(argv[++i]); }
        else if (std::strcmp(argv[i], "--b-max") == 0 && i+1 < argc){ b_max = std::atof(argv[++i]); }
        else if (std::strcmp(argv[i], "--reward-every") == 0 && i+1 < argc){ reward_every = std::atoi(argv[++i]); }
    ...
    for (int s=0; s<STEPS; ++s){
        ...
        if (reward_every > 0 && (s % reward_every) == 0){
            CHECK_CUDA(cudaMemset(d_reward, 0, sizeof(unsigned long long)));
            reward_counter_kernel<<<gb,tb>>>(dU + 0*cells, d_phi,
                                             dU + 3*cells, dU + 4*cells,
                                             (int)cells, rho_min, rho_max, phi_max, b_max,
                                             d_reward);
            unsigned long long h_reward=0;
            CHECK_CUDA(cudaMemcpy(&h_reward, d_reward, sizeof(unsigned long long), cudaMemcpyDeviceToHost));
            double frac = double(h_reward) / double(cells);
            printf("step %d reward %llu (%.3f of cells)\n", s, (unsigned long long)h_reward, frac);
        }
    }

    poisson.destroy();
    cudaFree(dU); cudaFree(dV); cudaFree(d_phi); cudaFree(d_reward);
    return 0;
}
